# pulumi-package Pulumi Component Provider (Go)

This repo is a boilerplate showing how to create a Pulumi component provider written in Go. You can search-replace `pulumi-package` with the name of your desired provider as a starting point for creating a component provider for your component resources.
...
Note that the generated provider plugin (`pulumi-resource-pulumi-package`) must be on your `PATH` to be used by Pulumi deployments. If creating a provider for distribution to other users, you should ensure they install this plugin to their `PATH`.
...
$ cd examples/simple
$ yarn install
$ yarn link @pulumi/pulumi-package
...
The `pulumi-package` provider's plugin binary must be named `pulumi-resource-pulumi-package` (in the format `pulumi-resource-<provider>`).
...
```json
"resources": {
    "pulumi-package:index:StaticPage": {
        "isComponent": true,
        "inputProperties": {
            "indexContent": {
                "type": "string",
...
The component resource's type token is `pulumi-package:index:StaticPage` in the format of `<package>:<module>:<type>`. In this case, it's in the `pulumi-package` package and `index` module. This is the same type token passed to `RegisterComponentResource` inside the implementation of `NewStaticPage` in `provider/pkg/provider/staticPage.go`, and also the same token referenced in `Construct` in `provider/pkg/provider/provider.go`.
...
The provider makes this component resource available in the `construct` function in `provider/pkg/provider/provider.go`. When `construct` is called and the `typ` argument is `pulumi-package:index:StaticPage`, we create an instance of the `StaticPage` component resource and return its `URN` and state.
